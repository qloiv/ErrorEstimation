...
joints =[ (5*pi)/6, -(2*pi)/3, pi/3]


 %gelenkstellungen angeben (3)
arms = [1,1,1]
...
anglefk = my_fkin2(3); %letztes output argument ist der winkel
angle =atan2(T(2,1),T(1,1));  %das ist die formel aus dem paper, atan2 reihenfolge nicht vertuascht!
if(~ (angle==anglert && angle==anglefk))
    disp('no equal angles')
end
[my_ikin,my_ikin2] = ikin(my_fkin2(1),my_fkin2(2),anglefk,arms);
...
%rob.pprintlot(my_ikin,'notiles', 'top', 'noname','workspace', [-5,5,-5,5,0,5],'floorlevel', -1)
...
if(~ (isequal(T_ikine_sol,T_my_ikin_sol) || isequal(T_ikine_sol,T_my_ikin_sol2)))
    error('not equal')
end
% sampleSize = 20
